...
add_action( 'admin_bar_menu', __NAMESPACE__ . '\\clear_transients_node', 99 );
/**
 * Clear all transients with one click.
 *
 * @since 2.2.9
 */
function clear_transients_node($wp_admin_bar) {
	...
	if( isset($_GET['clear-transients']) && 1 === (int) $_GET['clear-transients'] ) {
		...
		wp_cache_flush();
		add_action( 'admin_notices', __NAMESPACE__ . '\\transients_cleared_notice' );
	}
	...
	if( isset($_GET['clear-orphans']) && 1 === (int) $_GET['clear-orphans'] ) {
		...
		add_action( 'admin_notices', __NAMESPACE__ . '\\orphans_cleared_notice' );
	}
	...
	if( isset($_GET['clear-ipq']) && 1 === (int) $_GET['clear-ipq'] ) {
		...
		add_action( 'admin_notices', __NAMESPACE__ . '\\ipq_cleared_notice' );
	}
 ...
 * @since 20170625
 */
function transients_cleared_notice() {
 ...
 * @since 20180604
 */
function orphans_cleared_notice() {
