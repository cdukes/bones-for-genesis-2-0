 ...
 * @param {string}   config.includeCredentials - Whether to include the user's session in the request. If false, WP will handle the request as if the user wasn't logged in.
 ...
 * @param {Function} config.onSuccess          - Function to call if request is successful, as determined by the server's response code. Passes the JSON-decoded response.
 * @param {Function} config.onError            - Function to call if request is unsuccessful, as determined by the server's response code (or a failed request). Passes the error string.
 * @param {Function} config.onComplete         - Function to call when request is completed, regardless of success or failure. Passes the JSON-decoded response or error string.
 */
export function ajax(config) {
	const { action, includeCredentials, data, onSuccess, onError, onComplete } = config;

	const url = `string` === typeof ajaxurl ? ajaxurl : document.body.dataset.ajax_url;
	...
	fetch(
		...
		.then((response) => {
			...
			if (200 > response.status || 299 < response.status) {
